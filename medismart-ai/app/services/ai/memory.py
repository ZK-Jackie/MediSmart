...
def _filter_keys(
    ...
    if before:
        keys = [
            k
            for k in keys
            if _parse_redis_checkpoint_key(k.decode())["checkpoint_id"] # noqa
...
class RedisSaver(BaseCheckpointSaver):
    ...
    def put_writes(
        ...
        Args:
            ...
            task_path (str): Path to the task creating the writes.
        """
        thread_id = config["configurable"]["thread_id"]
        checkpoint_ns = config["configurable"]["checkpoint_ns"]
        ...
        for idx, (channel, value) in enumerate(writes):
            ...
            if all(w[0] in WRITES_IDX_MAP for w in writes):
                ...
                for field, value in data.items():   # noqa
    ...
    def list(
            ...
            *,
            ...
            filter: Optional[dict[str, Any]] = None,    # noqa
...
class AsyncRedisSaver(BaseCheckpointSaver):
    ...
    async def _aget_checkpoint_key( # noqa
            self, conn, thread_id: str, checkpoint_ns: str, checkpoint_id: Optional[str]
    ) -> Optional[str]:
